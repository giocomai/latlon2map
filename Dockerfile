...
RUN apt-get update && apt-get install -y  gdal-bin git-core libcurl4-openssl-dev libgdal-dev libgeos-dev libgeos++-dev libgit2-dev libssh2-1-dev libssl-dev libudunits2-dev libxml2-dev make pandoc pandoc-citeproc zlib1g-dev libxt-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("rmarkdown",upgrade="never", version = "2.4")'
...
EXPOSE 3838
CMD  ["R", "-e", "options('shiny.port'=3838,shiny.host='0.0.0.0');latlon2map::ll_app(max_file_size = 500)"]
