...
ll_osm_it_gpkg[["comuni"]] <- ll_get_nuts_it(level = "lau") %>% 
  ...
  dplyr::left_join(y = link_comuni_pre %>% 
                     dplyr::mutate(PRO_COM_T = stringr::str_extract(string = link,
                                                                    pattern = "[[:digit:]]+")),
                   by = "PRO_COM_T") %>% 
  dplyr::transmute(name = COMUNE,
                   code = PRO_COM_T,
...
ll_osm_it_gpkg[["province"]] <- ll_get_nuts_it(level = 3) %>% 
  ...
  dplyr::left_join(y = link_province_pre %>% 
                     dplyr::mutate(COD_PROV = stringr::str_extract(string = link,
                                                                    pattern = "[[:digit:]]+")),
                   by = "COD_PROV") %>% 
  dplyr::transmute(name = DEN_UTS,
                   code = COD_PROV,
...
ll_osm_it_gpkg[["regioni"]] <- ll_get_nuts_it(level = 2) %>% 
  ...
  dplyr::left_join(y = link_regioni_pre %>% 
                     dplyr::mutate(COD_REG = stringr::str_extract(string = link,
                                                                   pattern = "[[:digit:]]+")),
                   by = "COD_REG") %>% 
  dplyr::transmute(name = DEN_REG,
                   code = COD_REG,
