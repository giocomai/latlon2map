...
ll_get_lau_nuts_concordance <- function(lau_year = 2019,
  ...
  corresondence_df <- purrr::map_dfr(.x = country_sheets,
                                     .f = function(current_sheet_name) {
                                       pb$tick()
                                       current_sheet <- readxl::read_xlsx(path = xlsx_file,
                                       ...
                                       current_sheet %>% 
                                         ...
                                         dplyr::transmute(country = current_sheet_name,
  ...
  saveRDS(object = corresondence_df,
          file = rds_file)
