#' Find the population-weighted centre of a municipality or other geographic
#' entity
#'
#' @param sf_location Relevant gepgraphic entity as an  `sf` object, typically
#'   retrieved with [ll_get_lau_eu()] or similar.
#' @param sf_population_grid Relevant population grid, typically retrieved with
#'   [ll_get_population_grid()].
#' @param power Defaults to 2. To give more weight to cells with higher
#'   population density, raise the number of residents by the power of.
#' @param join Defaults to [sf::st_intersects()].
#' @param adjusted If adjusted is set to `TRUE`, join is ignored. The population
#'   of cells along the boundary line are weighted by the share of the cell
#'   included within the border.
#'
#' @return A point as an `sf` object. The population-weighted centre for the
#'   relevant entity.
#' @export
#'
#' @examples
